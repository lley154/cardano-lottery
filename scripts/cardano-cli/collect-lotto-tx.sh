...
if [ -z $1 ]; 
then
    echo "collect-lotto-tx-.sh:  Invalid script arguments"
    echo "Usage: collect-lotto-tx.sh [devnet|testnet|mainnet]"
    exit 1
fi
ENV=$1
...
redeemer_file="$BASE/scripts/cardano-cli/$ENV/data/redeemer-lotto-collect.json"
...
jq -c ' .fields[5].int  |= 0 ' > $WORK/lotto-datum-out.json
